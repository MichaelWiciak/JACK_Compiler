...
Token PeekNextToken (){
	...
	Token token;
	token = GetNextToken ();
	// if the token is a string, then we need to put the quotes back
	if (token.tp==STRING){
		ungetc('"', inputFile);
	}
...
// this function is used to go back to the beginning of the file
// int ResetLexer (){
